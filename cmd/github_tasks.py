...
USER_SCENARIOS_HEADING_PATTERN = re.compile(
    r"^##\s+(?:User Scenarios\s*&\s*Testing|Escenarios de Usuario y Pruebas).*$",
    re.IGNORECASE,
)
NEXT_SECTION_PATTERN = re.compile(r"^##\s+.+$")
USER_STORY_HEADING_PATTERN = re.compile(
    r"^###\s+(?:User Story|Historia de Usuario)\s+(?P<number>\d+)\s*-\s*(?P<title>.+?)\s+\((?:Priority|Prioridad):\s*(?P<priority>P\d+)\)$",
    re.IGNORECASE,
)
...
def parse_user_stories(markdown: str) -> list[UserStory]:
    ...
    for line in lines:
        ...
        if stripped.startswith("**Why this priority**:") or stripped.startswith("**Por qué esta prioridad**:"):
            mode = "why_priority"
        ...
        if stripped.startswith("**Independent Test**:") or stripped.startswith("**Prueba Independiente**:"):
        ...
        if stripped.startswith("**Acceptance Scenarios**:") or stripped.startswith("**Escenarios de Aceptación**:"):
...
def gh_issue_create(repository: str, title: str, body: str, labels: list[str]) -> dict:
    ...
    # gh issue create outputs the url as the last string block
    # Sample output:
...
def build_issue_body(story: UserStory, spec_path: Path) -> str:
    ...
    return (
        f"Generado automaticamente desde `{spec_path.name}` (task.md).\n\n"
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Lee user stories de task.md y crea issues en GitHub usando gh CLI."
    )
    parser.add_argument("task_path", help="Ruta al archivo task.md")
    parser.add_argument(
        "--repo",
